...
public sealed class LocalLegacyOverlapApprovalTests
{
    ...
    public async Task ApprovalIncludingComplianceOverrideCannotAcceptExistingOverlappingTime(bool useOverride, bool overlaps)
    {
        ...
        await using (var db = factory.CreateDbContext())
        {
            ...
            if (useOverride)
            {
                ...
                person.Forms.Add(new Form(
                    ...
                    targetEffectiveDate: DateTime.Today.AddYears(-1).Date));
                await db.SaveChangesAsync();
            }
        ...
        if (useOverride)
        {
            // The exception must name the exact blockers the service itself reports.
            var blocked = await service.GetNonCompliantNotesAsync(reviewer.Id);
            blockerIds = blocked.Single(row => row.Id == noteId)
                .ComplianceBlockers.Select(blocker => blocker.ObligationId).ToArray();
            Assert.NotEmpty(blockerIds);
        }
        Func<Task> approve = () => useOverride
            ? service.ApproveWithOverrideAsync(noteId, reviewer.Id,
                "A compliance exception cannot override time.", revision,
                blockingObligationIds: blockerIds, attestationConfirmed: true)
