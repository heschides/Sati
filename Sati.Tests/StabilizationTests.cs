...
public sealed class StabilizationTests
{
    ...
    public void ReleaseNotesMatchTheDemoAssemblyVersionAndDocumentProductionGaps()
    {
        ...
        Assert.Equal("1.3.23", version);
        Assert.Equal(version, apiVersion);
        Assert.Equal("Form dates and billing follow the work recorded", ProductReleaseNotes.ReleaseName);
        ...
        Assert.Contains(ProductReleaseNotes.Sections, section =>
            section.Title == "Marking a day with no billable work" &&
            section.Items.Any(item => item.Contains("counts in Documented Avg as a zero", StringComparison.OrdinalIgnoreCase)));
        // A supervisor seeing something new about a case manager is said plainly, to them.
        Assert.Contains(ProductReleaseNotes.Sections, section =>
            section.Title == "Supervisors see settled days with no billable work");
        // 1.3.17's reasons stay described, because this release carries them too.
        Assert.Contains(ProductReleaseNotes.Sections, section =>
    ...
    public void PersonCreationGeneratesOneFormPerType()
    {
        ...
        Assert.Equal(PersonSaveRules.FormTypes.Count, person.Forms.Count);
        Assert.DoesNotContain(person.Forms, form => form.Type is
