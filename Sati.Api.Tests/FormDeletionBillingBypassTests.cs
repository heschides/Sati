...
public sealed class FormDeletionBillingBypassTests(SatiApiFactory factory)
{
    ...
    public async Task DeletingAnUnattestedOverdueFormCannotRemoveItsBillingBlock()
    {
        ...
        var formId = await AddFormForNoteAsync(noteId, "PCP", DateTime.Today.AddDays(-2));
        ...
        Assert.Contains("current compliance requirements", await before.Content.ReadAsStringAsync());
        ...
        Assert.Contains("current compliance requirements", await after.Content.ReadAsStringAsync());
    }
