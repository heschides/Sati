...
public sealed class ConsumerPermissionRevocationTests(SatiApiFactory factory)
{
    ...
    private async Task<CaseworkData> SeedAsync()
    {
        ...
        var form = await db.Forms.FirstAsync(x => x.PersonId == personId && x.Type == "PCP");
        // Keep this permission-boundary fixture inside the real PCP opening window.
        // The opening endpoint now owns the audited transition and correctly rejects
        // arbitrary OpenedDate changes through the compatibility update route.
        form.TargetEffectiveDate = DateTime.Today;
        form.DueDate = DateTime.Today;
    ...
    private static Task<HttpResponseMessage> SendAsync(HttpClient client, string operation, CaseworkData data) =>
        operation switch
        {
            ...
            "form-open" => client.PostAsJsonAsync(
                $"/api/v1/forms/{data.FormId}/open",
                new OpenFormRequest(DateTime.Today)),
