...
| Notes | `POST /notes` | Note person's assigned user and agency; selected Form belongs to the same person and agency | Own caseload only; note agency is assigned server-side. A newly Logged non-release Form note must name its exact obligation; its activity date attests that form in the same transaction. |
