| Billing | `GET /billing/periods/{periodId}/claims` | Billing period user's `AgencyId` | Billing permission; returns each claim of the actor agency's period with where it stands and which corrections `ClaimCorrectionRules` allows, assembled from retained files, recorded 999/277CA verdicts, and 835 outcomes. Carries the client name and charge, never note narrative. |
| Billing | `POST /billing/periods/{periodId}/corrections` | Billing period user's `AgencyId`; source note and person must remain in that agency | Billing permission and a re-validated current actor, inside the period write scope. Only a submitted period. The requested action must be one `ClaimCorrectionRules` allows for that claim's own history, and a replacement or void must cite the payer claim number the 835 gave. A resend or replacement is rebuilt from current client and agency records and must pass the same candidate and 837P-readiness checks as a new claim line; a void repeats the adjudicated claim exactly. The correction row is append-only. |
| Billing | `POST /billing/periods/{periodId}/corrections/edi` | Billing period user's `AgencyId` | Billing permission and a re-validated current actor. Generates one 837P containing only that period's corrections not yet sent in the requested test/production mode, with CLM05-3 frequency 1, 7, or 8 and REF*F8 for 7 and 8. Idempotency keys are not shared with the original-file route. The retained file and its correction links are immutable. |
| Billing | `GET /billing/remittance-deposits/{depositId}/eft` | Deposit and entry `AgencyId` | Billing permission; returns the append-only entries recorded against one of the actor agency's deposits, with who recorded each. A foreign deposit is 404. |
...
| Admin | `POST /admin/demo/reset` | Whole validated Demo environment | Administration permission and exact `SatiDemo` / `Demo` server identity. Requires the literal `RESET DEMO` confirmation and delegates to the separately authenticated reset Function; its host key is a protected API setting and the client never receives that key or database authority. The Function validates the request and queues it (202); the route returns "Reset started" without waiting, because a full reset outlasts the HTTP front end. A queue worker then restores the approved synthetic baseline, rotates the database instance id to invalidate every prior token, reruns the rolling-date seed, and completes past compliance history, while Demo mutations are excluded. A failed reset is never retried automatically. The outcome is recorded as a `demo.reset.completed` or `demo.reset.failed` audit event in the requesting Admin's agency, so it is read from recent activity after signing in again. Production returns 404. |
...
| People | `PUT /people/{personId}` | Person's assigned user and agency | Own caseload only, with current persisted ownership/permission rechecked in the write transaction. The creation-time test-data classification is immutable. An effective-date change is refused after any stored form or release obligation exists, pending an audited schedule reconciliation. Relevant signer changes atomically cancel unfinished invitations or stop signed-copy portal access. |
...
| Notes | `POST /notes` | Note person's assigned user and agency; selected Form belongs to the same person and agency | Own caseload only; note agency is assigned server-side. A newly Logged non-release Form note must name its exact obligation; its activity date attests that form in the same transaction. Selecting an older annual target after an incomplete renewal becomes available requires a written justification and records a minimized audit event; evidence is never redirected. |
| Notes | `PUT /notes/{id}` | Current and requested note persons' assigned user and agency; selected Form belongs to the same person and agency | Both the stored note and any requested reassignment target must belong to the actor's own caseload and agency; server enforces the `NoteWorkflow` transition table, rejects stale revisions, and audits a successful reassignment. Logging a non-release Form note also validates and attests its exact obligation atomically, including the older-cycle written-justification rule. |
...
| Counted service days | `GET /service-day-inclusions/{year}?userId=` | Inclusion `UserId` | Own user, or a case manager the actor can reach: gated by `TenantAccess.CanAccessUserAsync` before the id reaches the query. |
...
| Billing | `POST /billing/claim-lines` | Source note person's owning user's `AgencyId` | Billing permission; source note must be approved and in actor agency. The API re-evaluates its service-date policy and exact blockers, including any retained one-note Supervisor exception or Admin recovery whose frozen evidence still matches, then validates the exact constructed frozen row with the shared 837P-readiness rule before saving it. Non-release Form work must also match its exact attested obligation and be completed by that obligation's due date; the generic exception and recovery cannot waive this condition. |
...
| Forms | `GET /people/{personId}/forms/{type}/attestations` | Form person's assigned user and agency | Accessible case manager only through `TenantAccess.CanAccessPersonAsync`; the supplied form id must belong to the route person and type. Returns the append-only attestation/revocation history with server-resolved actor labels. |
...
| Form review flags | `GET /form-attestation-change-review-flags?audience=supervisor\|billing` | Flag and person `AgencyId`; supervisor results additionally require a same-agency current case manager in the actor's supervisory scope | The supervisor audience requires Supervision and returns only its assigned caseload unless agency-wide supervision is granted. The billing audience requires Billing and returns only billing-attention flags for the actor's agency. The route is read-only. |
| Admin form-note correction | `GET /admin/notes/{noteId}/form-date-correction-target` | Note, person, owning case manager, and exact form must belong to the actor's agency | Administration permission. Returns the submitted note's revision, activity date, live or recently revoked form completion, due date, and claim-presence fact for an exact linked non-release Form note. |
| Admin form-note correction | `POST /admin/notes/{noteId}/correct-form-date` | Note, person, owning case manager, and exact form must belong to the actor's agency | Administration permission. Requires current revision, corrected work date, written reason, and source-evidence confirmation. A serializable transaction corrects the note and attestation ledger and records an audit event and review flags. If a claim line exists, its original service date is retained and billing is flagged for review. It cannot waive a genuinely late form-work billing hold. |
| Admin release-note correction | `GET /admin/notes/{noteId}/release-date-correction-target` | Note, person, owning case manager, and exact release obligation must belong to the actor's agency | Administration permission. Returns a submitted manual-attestation note's revision, activity date, release completion, due date, and claim-presence fact. Electronic-signature evidence is excluded. |
| Admin release-note correction | `POST /admin/notes/{noteId}/correct-release-date` | Note, person, owning case manager, and exact release obligation must belong to the actor's agency | Administration permission. Requires current revision, corrected work date, written reason, and source-evidence confirmation. A serializable transaction corrects the note and manual release attestation and records audit and review flags. Existing claim service dates remain intact and the claim line is flagged for billing review. |
...
| Release obligations | `POST /people/{personId}/release-obligations/{obligationId}/attest` | Person's and obligation's `AgencyId` | Accessible caseload only; route person and opaque obligation id must match. Records manual attestation with validated actual/available dates and server-derived actor/UTC time, linking an exact same-date note or creating a Pending draft atomically. A conflicting note date is refused. No artifact is required. |
| Release obligations | `POST /people/{personId}/release-obligations/{obligationId}/attestation/revoke` | Person's and obligation's `AgencyId` | Accessible caseload only; a reason is required. Revokes the active manual attestation with actor and UTC history while retaining the note and release record. Electronic signatures require their separate correction path. |
